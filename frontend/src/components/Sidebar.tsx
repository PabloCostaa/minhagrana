...
import { Home, Tag, Target, BarChart3, Plus, Settings, ChevronLeft, ChevronRight } from 'lucide-react';
...
const navigation = [
  ...
  { id: 'add-expense', label: 'Adicionar', icon: Plus },
    ...
    >
      ...
      <div className="p-6 border-b border-light-border dark:border-dark-border flex items-center justify-between">
        ...
        >
          {!isCollapsed && (
            <div>
              <h1 className="text-lg font-bold text-lime-accent font-editorial">ExpenseTracker</h1>
              <p className="text-xs text-light-text-secondary dark:text-dark-text-secondary">Controle de Gastos</p>
            </div>
          )}
        </motion.div>
        <button
        ...
        <div className="flex items-center space-x-3 p-3 bg-light-glass dark:bg-dark-glass rounded-xl">
          ...
          {!isCollapsed && (
            ...
            >
              ...
              <p className="text-xs text-light-text-secondary dark:text-dark-text-secondary">Usuário Premium</p>
            </motion.div>
          )}
        </div>
      </div>
